...
try:
    ...
    if os.path.exists(SYMPTOM_PATH) and os.path.getsize(SYMPTOM_PATH) > 0:
        ...
        if isinstance(symptom_columns, (list, tuple)):
            symptom_columns = [str(col) for col in symptom_columns if col and not str(col).startswith("Unnamed")]
            if model is not None and hasattr(model, 'feature_names_in_'):
                symptom_columns = [col for col in symptom_columns if col in model.feature_names_in_]
        print(f"✓ Symptom columns loaded: {len(symptom_columns)} symptoms")
    if os.path.exists(LABEL_ENCODER_PATH) and os.path.getsize(LABEL_ENCODER_PATH) > 0:
...
def predict_disease(symptoms_text):
    ...
    try:
        ...
        # If labels are encoded, map them back to readable disease names
